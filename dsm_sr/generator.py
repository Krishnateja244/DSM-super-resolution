...
class SrganGenerator(nn.Module):
  ...
  def forward(self, x):
    ...
    if self.scale ==2:
        ...
        x12 = self.PS1(x11)
        x13 = self.PS2(x12)
    return x13 #self.final_layer(x13)
...
class SRCNN(nn.Module):
    ...
    def forward(self, x,im):
        ...
        x6_rgb = self.feature_extractor(torch.cat([im, x_bicubic-x_bicubic.mean((1,2,3), keepdim=True) ], 1))
        x10 = self.last_afconcat(x1+x6_rgb.detach())
        x7_rgb = self.last_afconcat(x6_rgb)
        return x10,x7_rgb
    
class unet_rgb(nn.Module):
    def __init__(self, num_channels=4,feat_channels=64):
        super(SRCNN, self).__init__()
        self.feature_extractor =  torch.nn.Sequential(
                smp.Unet('resnet50', classes=feat_channels, in_channels=num_channels))
        self.upsample_torch = torch.nn.Upsample(scale_factor=4, mode='bicubic')
        self.last_afconcat = nn.Sequential(
            nn.Conv2d(64,1,3,1,1),
        )
